...
function findDeviationLimits(weight: number, sdArray: []) {
  let lowerLimitIn;
  let higherLimitIn;
...
export default function getZScoreWFA(
  ageInMonths: number,
  weight: number,
  gender: any
) {
  let map: any = femaleMap;
  ...
  const { higherLimitIn, lowerLimitIn }: any = findDeviationLimits(
    weight,
    sdArray
  );
