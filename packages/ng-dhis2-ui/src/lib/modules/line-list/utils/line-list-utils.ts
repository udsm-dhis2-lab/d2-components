...
export const getProgramStageData = (
  ...
  const entityColumns = Array.from(allDataElements).map((dataElementId: string) => {
  ...
  const dataElementsData: TableRow[] = events.map((event: any, idx: number) => {
    ...
    allDataElements.forEach((dataElementId: string) => (row[dataElementId] = ''));
...
export const getEventData = (
  ...
  const dataElementsData = events.map((event: any, idx: number) => {
    ...
    uniqueDataElements.forEach((dataElement: string) => (row[dataElement] = ''));
