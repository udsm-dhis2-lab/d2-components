...
export class FieldComponent implements OnInit, OnChanges, OnDestroy {
  ...
  previousTravelsArray: Array<any> = [];
  previousTravelsNumber: any;
  ...
  get fieldError(): string | undefined {
    ...
    if (error['min']) {
      return `${this.field.label} should not be less than ${error['min'].min}!`;
    } else if (error['max']) {
      return `${this.field.label} should not be greater than ${error['max'].max}!`;
